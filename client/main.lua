...
local function SpawnVehicle()  
    ...
    if not DoesEntityExist(vehhash) then
        jobVeh = CreateVehicle(vehhash, spawnPos, spawnHeading, true, false)                        
        ClearAreaOfVehicles(GetEntityCoords(jobVeh), 5000, false, false, false, false, false);  
...
local function MechanicNPC()
	...
	TriggerServerEvent('mh-aiems:server:FixVehicle', QBCore.Functions.GetPlate(playerVeh))
	Citizen.Wait(5000)
	LeaveTarget()
	IsSpawning = true
    end)
    ...
    while true do
      Citizen.Wait(100)
		...
		if IsActive then
			...
			dist1 = Vdist(engine.x, engine.y, engine.z, ld.x, ld.y, ld.z)
		    end
